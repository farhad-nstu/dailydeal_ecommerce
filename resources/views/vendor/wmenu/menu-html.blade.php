...
@if(isset($indmenu))
<div class="card float-right col-md-6" style="
							    margin-top: -65px;
							">
	<form action="{{route('menu.wpmenulocation')}}" method="post">
	@csrf
	  <div class="card-body">

	    Select <span class="text-primary"> {{$indmenu->name}} </span> location

	    <div class="custom-control custom-checkbox">
		  <input type="checkbox" class="custom-control-input" id="primary" name="menu_locations[]" value="primary"

		  @php
		  	if (strpos($indmenu->location,"primary") > 0) {
		  	 	echo "checked=''";
		  	 }
		  @endphp

		  >
		  <label class="custom-control-label" for="primary">Primary Navigation</label>
		</div>

		<div class="custom-control custom-checkbox">
		  <input type="checkbox" class="custom-control-input" id="mobile" name="menu_locations[]" value="mobile"
		  @php
		  	if (strpos($indmenu->location,"mobile") > 0) {
		  	 	echo "checked=''";
		  	 }
		  @endphp
		  >
		  <label class="custom-control-label" for="mobile">Mobile Navigation</label>
		</div>

		<div class="custom-control custom-checkbox">
		  <input type="checkbox" class="custom-control-input" id="footer_right" name="menu_locations[]" value="footer_right"
		  @php
		  	if (strpos($indmenu->location,"footer_right") > 0) {
		  	 	echo "checked=''";
		  	 }
		  @endphp
		  >
		  <label class="custom-control-label" for="footer_right">Footer Right Navigation</label>
		</div>

		<div class="custom-control custom-checkbox">
		  <input type="checkbox" class="custom-control-input" id="footer_left" name="menu_locations[]" value="footer_left"
		  @php
		  	if (strpos($indmenu->location,"footer_left") > 0) {
		  	 	echo "checked=''";
		  	 }
		  @endphp
		  >
		  <label class="custom-control-label" for="footer_left">Footer Left Navigation</label>
		</div>


	    <input type="hidden" name="menu_id" value="{{$indmenu->id}}">
	    <button type="submit" class="button button-primary menu-save">Update</button>
...
<div class="row"></div>
	...
	<br>
	<form action="{{route('menu.wpmenu.bd.store')}}" method="post">
		@csrf
		<label class="menu-name-label howto open-label" for="menuname_bd"> <span>মেনুর নাম &nbsp;</span>
		<input name="menuname_bd" id="menuname_bd" type="text" class="menuname_bd regular-text menu-item-textbox" title="Enter menu name" value="@if(isset($indmenu)){{$indmenu->name_bd}}@endif">
		<input type="hidden" id="idmenu" value="@if(isset($indmenu)){{$indmenu->id}}@endif" name="idmenu" />
	</label>&nbsp;&nbsp;
	<button type="submit" class="button button-primary menu-save">Update</button>
	</form><br>
	<table class="table">
	  <thead class="thead-dark">
	    <tr>
	      <th scope="col">Menu Name English</th>
	      <th scope="col">Menu Name Bangla</th>
	  ...
	  <tbody>
		...
		<tr>
		<form action="{{route('menu.wpmenuoption.bd.store')}}" method="post">
		@csrf
	      <td>{{$menu->label}}</td>
	      <td><input type="text" name="menuname_items_bd" value="{{$menu->label_bd}}"></td>
	      <td><button type="submit" class="btn">Update</button></td>
	      <input type="hidden" name="menu_option_id" value="{{$menu->id}}">
	     </form>
	    </tr>
